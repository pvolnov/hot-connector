    ...
    <div className={"input-form"}>
      ...
      <div className={"input-row grid grid-cols-2 gap-4"}>
        <div className={"input-group"}>
          ...
          <input className={"input-text"} type={"text"} value={receiverId} onChange={e => {
            setReceiverId(e.target.value);
          }}/>
        </div>
        <div className={"input-group"}>
          <p className={"input-label"}>Amount to Send</p>
          <input className={"input-text"} value={amount} type={"number"} step={"0.01"} onChange={e => {
            setAmount(e.target.value);
          }}/>
        </div>
        <button className={"input-button"} onClick={() => sendTx()}>Send tx</button>
        {lastResult != null && (
          <FinalOutcome outcome={lastResult} network={network}/>
        )}
      </div>
    </div>
  );
